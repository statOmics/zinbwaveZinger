...
zingeREdgeRWeights.pfun=function(counts, group, design=NULL, mc.cores=2, niter=200){
	library(edgeR) ; library(genefilter)
	d <- DGEList(counts = counts, group = group )
	d <- edgeR::calcNormFactors(d)
    design = model.matrix(~ group)
...
MAST.pfun <- function(counts, group, design=NULL, mc.cores=2, niter=NULL){
    ...
    tpm = log2(tpm+1) #as in MAST vignette: "a matrix of log2 + 1 transcripts per million (TPM)"
    #sca <- FromMatrix('SingleCellAssay', t(tpm), cData=data.frame(group=group)) # old MAST version
    sca <- FromMatrix(tpm, cData=data.frame(group=group), fData=as.data.frame(rownames(counts)))

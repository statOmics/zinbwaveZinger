...
#get subset of data
set.seed(23984)
simCells = sample(ncol(se),500)
seSim = se[,simCells]
set.seed(1)
rowsTenx = sample(nrow(tenxCompare),nrow(se))
dds_origTenx <- DESeqDataSetFromMatrix(countData = tenxCompare[rowsTenx,simCells],
                                  colData = data.frame(sample = colnames(seSim), row.names = colnames(seSim), stringsAsFactors = FALSE), design = ~ 1)
...
countsimQCReport(ddsList=list(original=dds_origIslam,zingeR=dds_zingerIslam),
                  outputFile="~/islamMoMPositiveUnnormalizedGoodComparisonImprovedEstimator.html",
...
countsimQCReport(ddsList=list(original=dds_origGSE74596,zingeR=dds_zingerGSE),
                outputFile="~/GSEMoMPositiveUnnormalizedGoodComparisonImrpovedEstimator.html",
...
## estimate parameters
groupTenx = rep(1,500)
design = model.matrix(~0 + groupTenx)
paramsTenx = getDatasetMoMPositive(counts=assays(seSim)$counts, design=design)
...
nSamples=ncol(seSim)
grp=as.factor(rep(0:1, each = nSamples/2)) #two-group comparison
nTags=nrow(seSim) #nr of features
set.seed(11)
DEind = sample(1:nTags,floor(nTags*.1),replace=FALSE) #10% DE
fcSim=(2 + rexp(length(DEind), rate = 1/2)) #fold changes
libSizes=sample(colSums(assays(seSim)$counts),nSamples,replace=TRUE) #library sizes
...
dds_zingerTenx <- DESeqDataSetFromMatrix(countData = tenxSim$counts, colData = data.frame( sample = colnames(tenxSim$counts), row.names = colnames(tenxSim$counts), stringsAsFactors = FALSE), design = ~ 1)

countsimQCReport(ddsList=list(original=dds_origTenx,zingeR=dds_zingerTenx), outputFile="~/tenxPositiveNormalizedGoodComparison.html", description="tenx dataset, normalized Method of Moments on positive counts")

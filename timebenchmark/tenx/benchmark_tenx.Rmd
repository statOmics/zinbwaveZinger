...
runEdgeR <- function(e) {
  ...
  edger.lrt <- glmLRT(edger.fit, coef="condition2")
...
runZinbwaveDESeq2 <- function(e){
  ...
  resultsNames(dse)
  res = results(dse, name="condition2")
...
runMAST <- function(e){
  ...
  thres <- thresholdSCRNACountMatrix(assay(sca), nbins = 300, conditions = pData(e)$condition)
  #par(mfrow=c(5,4))
  #plot(thres)
...
runVoom <- function(e) {
  ...
  fit <- lmFit(v,design)
  fit <- eBayes(fit)
  tt <- topTable(fit,coef="condition2",n=nrow(dgel),sort.by="none")
  pvals <- tt$P.Value
  padj <- p.adjust(pvals,method="BH")
  padj[is.na(padj)] <- 1
...
runDESeq2 <- function(e, retDDS=FALSE) {
  ...
  dds <- DESeq(dds,betaPrior=TRUE,quiet=TRUE, minReplicatesForReplace=Inf) # imputation step disabled
  res <- results(dds)
...
runSCDE <- function(e){
  ...
  rownames(counts) = rownames(fData(e))
...
colData(tenx)$condition = factor(colData(tenx)$seurat)
tenx = tenx[sample(nrow(tenx), 10000), ]
# convert to expressionSet
tenx <- as(tenx, "ExpressionSet")
dim(tenx)
```
...
## Time benchmarking
```{r res}
algos <- list("limma-voom"      = runVoom,
              "MAST"            = runMAST,
              "edgeR"           = runEdgeR,
              "edgeR-zinbwave"  = runZinbwaveEdgeR,
              "DESeq2"          = runDESeq2_poscounts)

algos <- list("DESeq2-zinbwave" = runZinbwaveDESeq2,
              "SCDE"            = runSCDE)
...
tt <- lapply(namesAlgos, function(n){
  print(n)
  timing = system.time(algos[[n]](tenx))
  save(timing, file = sprintf('benchmark_tenx_%s.rda', n))
  timing
})
```
...
cpuTime_tenx <- sapply(namesAlgos, function(n){
  load(sprintf('benchmark_tenx_%s.rda', n))
  timing[[3]]
})
```

...
usoskin <- usoskin[sample(nrow(usoskin), 10000), ]
...
## Time benchmarking
```{r res}
algos <- list("DESeq2-poscounts"= runDESeq2_poscounts,
              "edgeR"           = runEdgeR,
...
tt <- lapply(namesAlgos, function(n){
  print(n)
  timing = system.time(algos[[n]](usoskin))
  save(timing, file = sprintf('benchmark_usoskin_%s.rda', n))
  timing
})
```
...
```{r}
cpuTime_usoskin <- sapply(namesAlgos, function(n){
  load(sprintf('benchmark_usoskin_%s.rda', n))
  timing[[3]]
})
```
...
dfTime = data.frame(time = cpuTime_usoskin,
                    method = namesAlgos)

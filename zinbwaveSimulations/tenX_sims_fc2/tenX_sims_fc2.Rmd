...
source(paste0(pathToParentFolder,"zingeRsimulations/simulationHelpFunctions_v7_diffInZero.R"))
```
...
Fit between simulated and real dataset is not great, but Koen is working on new simulation framework.
...
zingeR_edgeR <- function(counts, group, ylim = NULL, xlim = NULL){
  ...
  d <- suppressWarnings(edgeR::calcNormFactors(d))
  design <- model.matrix(~ group)
  weights <- zingeR::zeroWeightsLS(counts = d$counts, design = design, maxit = 100, normalization = "TMM", verbose = TRUE, plotW=TRUE)
  d$weights <- weights
  d <- estimateDisp(d, design)

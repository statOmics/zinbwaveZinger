...
```{r}
params = getDatasetZTNB(counts = counts, 
...
simData <- NBsimSingleCell(foldDiff = fcSim, ind = DEind,
                           dataset = tenxHlp, nTags = nTags,
                           group = grp,
                           verbose = TRUE, params = params,
...
```{r}
# BCV plots
dOrig=suppressWarnings(edgeR::calcNormFactors(DGEList(counts)))
...
plot(x=log(colSums(counts)), y=colMeans(counts==0), xlab="Log library size", ylab="Fraction of zeros", xlim=c(5.5,20))
points(x=log(colSums(simData$counts)), y=colMeans(simData$counts==0), col=2)
```

```{r}
# association of aveLogCPM with zeros
plot(x=edgeR::aveLogCPM(counts), y=rowMeans(counts==0), xlab="Average log CPM", ylab="Fraction of zeros", ylim=c(0,1), col=alpha(1,1/2), pch=19, cex=.3)

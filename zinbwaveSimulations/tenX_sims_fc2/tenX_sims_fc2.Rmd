...
dOrig=suppressWarnings(edgeR::calcNormFactors(DGEList(tenXData)))
dOrig = estimateDisp(dOrig, design=model.matrix(~cellType))
fitOrig=glmFit(dOrig,design=model.matrix(~cellType))

plot(x=log(rowMeans(tenXData)), y=rowMeans(tenXData==0), pch=16, cex=1/5, ylab="Fraction of zeros", xlab="log mean count")
grid=seq(min(rowMeans(tenXData)),max(rowMeans(tenXData)),length.out=5000)
# poisson
lines(x=log(grid), y=dpois(0,lambda=grid), col="gold", lwd=3)
# NB grid
lines(x=log(grid), y=dnbinom(0,mu=grid, size=1/dOrig$common.dispersion), col="steelblue", lwd=3)
legend("topright",c("Poisson","NB"), lty=1, col=c("gold","steelblue"), lwd=3)


# use the real data to plot the smooth line using smoothers. allows to use gene-specific dispersion and pi.

# NB real data
zeroProbData = dnbinom(0,mu=fitOrig$fitted.values, size=matrix(1/dOrig$tagwise.dispersion,nrow=nrow(fitOrig),ncol=ncol(fitOrig),byrow=FALSE))
plot(x=log(rowMeans(tenXData)), y=rowMeans(tenXData==0), pch=16, cex=2/5, ylab="Fraction of zeros", xlab="log mean count")
#NB
points(x=log(rowMeans(tenXData)), y=zeroProbData, pch=16, cex=2/5, col="steelblue")
#ZINB
coreReal = SummarizedExperiment(as.matrix(tenXData), colData=data.frame(cellType))
zinbCReal = zinbFit(coreReal, X="~cellType", epsilon=1e12, commondispersion=TRUE)
wReal=computeObservationalWeights(zinbCReal, tenXData)
dWOrig=suppressWarnings(edgeR::calcNormFactors(DGEList(tenXData)))
dWOrig$weights = wReal
dWOrig = estimateDisp(dWOrig, design=model.matrix(~cellType))
fit=glmFit(dWOrig,design=model.matrix(~cellType))
pi = t(getPi(zinbCReal))
zeroProbDataZINB = rowMeans(pi + (1-pi)*dnbinom(0, mu=fit$fitted.values, size=1/dWOrig$tagwise.dispersion))
points(x=log(rowMeans(tenXData)), y=zeroProbDataZINB, pch=16, cex=2/5, col="red")




## MA-like plot
plot(x=1-rowMeans(tenXData==0), y=rowMeans(tenXData==0)-rowMeans(zeroProbData), pch=16, cex=.2, xlab="Observed fraction of positive counts", ylab="Difference between observed and expected fraction of zeros", main="NB") ; abline(h=0,col=2,lwd=2)



plot(x=zeroProbData[,1], y=(tenXData[,1]==0)-zeroProbData[,1], pch=16, cex=1/3, xlab="Expected NB zero probability", ylab="Difference expected vs. true") ; abline(h=0,col=2,lwd=2)

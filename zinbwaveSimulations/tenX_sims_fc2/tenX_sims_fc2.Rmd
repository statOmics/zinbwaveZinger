...
MAST <- function(counts, group, ylim = NULL, xlim = NULL){
  ...
  list(pval = pvalAll, padj = padjAll, logfc = NA)
}
...
zingeR_edgeR <- function(counts, group, ylim = NULL, xlim = NULL){
  ...
  d <- suppressWarnings(edgeR::calcNormFactors(d))
  design <- model.matrix(~ group)
  weights <- zingeR::zeroWeightsLS(counts = d$counts, design = design, maxit = 500, normalization = "TMM", verbose = TRUE, plotW=FALSE)
  d$weights <- weights
  d <- estimateDisp(d, design)
...
# epsValues = c(1e8, 1e10, 1e12, 1e14)
# name=as.character(c(8, 10,12,14))
# for(i in 1:3){
#   assign(paste0("zinb_c_eps",name[i]), zinbFit(core, X = '~ grp', commondispersion = TRUE, epsilon=epsValues[i]))
# }
...
res[['ZINB_WaVE_edgeR_common']]  = zinbwave_edgeR(counts, grp, zinb_c, ylim=ylim, main = 'ZINB-WaVE, common dispersion', xlim = xlim)
...
## add high eps
res[['ZINB_WaVE_edgeR_common8']]  = zinbwave_edgeR(counts, grp, zinb_c_eps8, ylim=ylim, main = 'ZINB-WaVE, common dispersion', xlim = xlim)
res[['ZINB_WaVE_edgeR_common10']]  = zinbwave_edgeR(counts, grp, zinb_c_eps10, ylim=ylim, main = 'ZINB-WaVE, common dispersion', xlim = xlim)
res[['ZINB_WaVE_edgeR_common12']]  = zinbwave_edgeR(counts, grp, zinb_c_eps12, ylim=ylim, main = 'ZINB-WaVE, common dispersion', xlim = xlim)
res[['ZINB_WaVE_edgeR_common14']]  = zinbwave_edgeR(counts, grp, zinb_c_eps14, ylim=ylim, main = 'ZINB-WaVE, common dispersion', xlim = xlim)
...
zinbwave_c_weights_highEps <- computeZinbwaveWeights(zinb_c_highEpsilon, counts)
hist(zinbwave_c_weights_highEps[simData$dropout==0], main="ZINB-WaVE common: post prob for dropout", breaks=seq(0,1,0.05), cex.main=2/3)
hist(zinbwave_c_weights_highEps[simData$dropout==1], main="ZINB-WaVE common: post prob for NB zero", breaks=seq(0,1,0.05), cex.main=2/3)
...
##### TEMP
pp = COBRAData(pval = data.frame(edgeR=edgeResNewNotNorm[,"pval"],
        zinbwaveEdgeRes = zinwaveEdgeResNewNotNorm[,"pval"],
        #zinger_edger=hlp[,"pval"],
        row.names=rownames(simData$counts)),
               padj = data.frame(edgeR=edgeResNewNotNorm[,"padj"], zinbwaveEdgeRes = zinwaveEdgeResNewNotNorm[,"padj"], #zinger_edger=hlp[,"padj"],
                row.names=rownames(simData$counts)),
               truth = data.frame(status = trueDE, logMean = round(log(rowMeans(simData$counts)),2),
                simDisp = simData$Dispersion[,1],
                row.names=rownames(simData$counts)))
cobraperf <- calculate_performance(pp, binary_truth = "status", thrs = 0.05, splv="logMean", maxsplit=3)
cobraplot <- prepare_data_for_plot(cobraperf, colorscheme = "Paired")
...
plot_fdrtprcurve(cobraplot, plottype = c("curve", "points"), pointsize = .2, linewidth = .5, xaxisrange = c(0, .4), yaxisrange=c(0,0.4)) +
  theme(axis.text.x = element_text(size = 10),
        axis.text.y = element_text(size = 10),
        axis.title.x = element_text(size = 15),
        axis.title.y = element_text(size = 15),
        legend.text=element_text(size=7)) + theme(legend.position="bottom")

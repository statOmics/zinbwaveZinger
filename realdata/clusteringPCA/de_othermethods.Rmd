...
fit_edgeR <- function(counts, design, filter = NULL){
  ...
  de = de[, c('gene', 'PValue', 'padj', 'logFC', 'LR')]
  colnames(de) = c('gene', 'pval', 'padj', 'logfc', 'stat')
  de
}
```
...
fit_edgeR_zi <- function(counts, design, weights,
  ...
  de = de[, c('gene', 'PValue', 'padjFilter', 'logFC', 'LR')]
  colnames(de) = c('gene', 'pval', 'padj', 'logfc', 'stat')
  de
}
```
...
runLimmavoom <- function(counts, design) {
  ...
  padj[is.na(padj)] <- 1
  data.frame(gene = rownames(tt), pval=pvals, padj=padj, logfc=tt$logFC, stat = tt$t)
}
```
...
de$logfc[is.na(de$logfc)] = rnorm(n = sum(is.na(de$logfc)), 0, 0.0005) #no NA's allowed by fgsea and NAs with MAST
set.seed(6372)

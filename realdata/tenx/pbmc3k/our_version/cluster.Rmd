...
plot(w_tsne$Y, pch = 19, col = pal[colData(se)$seurat])
legend('bottomright', legend = unique(colData(se)$seurat), 
       fill = unique(pal[colData(se)$seurat]), cex = 0.5)
```
...
```{r tableclust}
table(primaryClusterNamed(ce), colData(se)$seurat)
```
...
```{r}
# todo: add deseq2
de = rbind(edgeR, deseq2, seurat, voom, mast, 

...
register(DoparParam())
print(system.time(zinb <- zinbFit(core, X = '~ ours', epsilon = 1e12)))
save(zinb, file = 'zinbX_eps12.rda')
```

```{r weightszinb}
load('zinbX_eps12.rda')
counts = assay(core)
...
runMAST <- function(counts, design){
  ...
  fit <- zlm(~ cngeneson + group, sca = sca, method = "bayesglm",
             ebayes = TRUE)

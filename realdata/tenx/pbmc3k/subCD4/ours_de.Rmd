...
zinbwave_deseq2 <- fit_DESeq2_zi(counts, design, weights = weights_zinbwave)
...
colnames(seurat) = colnames(mast)
```

# Concatenate all the result tables
```{r}
# zinbwave_deseq2, deseq2,
de = rbind(edgeR, seurat, voom, mast, zinbwave_edgeR)
...
gseaDf$method = factor(gseaDf$method, levels = c('edgeR', 'seurat', 'MAST', 'limmavoom', 'zinbwave_edgeR'))
...
```{r}
nes = gseaDf[grepl('CD4', gseaDf$pathway), 'NES']
limit = max(abs(min(nes)), max(nes))
ggplot(gseaDf[grepl('CD4', gseaDf$pathway), ], aes(method, pathway)) +
  geom_tile(aes(fill = NES)) + 
  scale_fill_gradient2(low = "blue", high = "red", mid = "white", 
   midpoint = 0, space = "Lab", name="Enrichment score", 
   limit = c(-limit, limit))
```
...
I think it should be adjusted pvalues instead. In edgeR and DESeq2, I think there are functions to filter genes with high fold change but low pvalue. TODO: look at that to improve volcano plots.
```{r}
par(mfrow=c(3,3))
for (x in unique(de$method)){
  temp = de[de$method == x, ]
  plot(temp[, 'logfc'], -log10(temp[, 'pval']),
       ...
       ylab = '-log10(pvalue)', xlab = 'logFC', main = x)
}
par(mfrow=c(1,1))
```

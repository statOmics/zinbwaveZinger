...
plot(x = logLib, y = pZero, xlab = "Log library size", ylab = "Fraction of zero's",
...
hist(weights_batch[assay(se) == 0], main = "", cex.lab = 1.5,
     xlab = "Posterior probability", cex.axis = 1.5, ylim=ylim)
hist(weights_nobatch[assay(se) == 0], add = TRUE, ylim=ylim,
     col = rgb(0.1, 0.8, 0.1, .2))
legend("topright", c("picking session + eff. lib. size",
                    "eff. lib. size"),
       fill = c(0, rgb(0.1, 0.8, 0.1, .2)), bty = "n", cex = 1.25)
mtext("c", at = -.25, font = 2, cex = 4/3)
```
```{r qqplotWeightsBatch}
qqplot(weights_batch[assay(se) == 0], weights_nobatch[assay(se) == 0],
       type = 'l', xlab='Posterior Probabilities - Batch', main = '',cex.lab = 1.5,cex.axis= 1.5,
       ylab='Posterior Probabilities - No Batch')
...
registerDoParallel(NCORES)
register(DoparParam())
res <- lapply(14:nreps, function(i) {
    cat(i," ")
...
```{r fprUsoskin}
ggplot(boxplotData[!grepl('zingeR', boxplotData$method),],
...
hh <- sapply(idx, function(i){
  hist(res[[1]][[i]]$pvals, main=names(algos)[i], cex.lab = 1.5,cex.axis= 1.5,cex.main=1.5,xlab='p-values',

...
set.seed(6389)
n=1e5
keep = sample(length(wb), n) #sample because R super slow with all the 50,000,000 values
df = data.frame(Weight = c(wb[keep], wob[keep]),
                col = c(rep('wb', n), rep('wob', n)))
df$col = factor(df$col)
pp = ggplot(df, aes(Weight, colour = col)) + stat_ecdf() +
  theme_bw() + ylab('ECDF(Weight)')
pp +
  scale_colour_manual(name="",
                      values = c(wb = 'darkgray',
                                 wob='darkgreen'),
                      labels = c("picking session + cell type",
                                 "cell type")) +
...
qqplot(weights_batch[assay(se)==0],
       weights_nobatch[assay(se)==0],
       type = 'l',
       xlab='Weights - Batch',
       main = '',cex.lab = 1.5,cex.axis= 1.5,
       ylab='Weights - No Batch')
abline(a=0,b=1,col='gray')
```

```{r mdplotWeightsBatch}
plotMD <- function(x, y, xlim = c(0,1), ylim = c(-1, 1),
                   main = ''){
  mm = .5*(x + y)
  dd = x - y
...
plotMD(unlist(weights_batch[assay(se)==0]),
       unlist(weights_nobatch[assay(se)==0]))
```
